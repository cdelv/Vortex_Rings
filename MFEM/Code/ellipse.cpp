...
void SaveState(ParMesh *pmesh, ParGridFunction *Velocity, int pid, int N){
    ...
    std::string path = "results/"+std::to_string(N)+"ellipse";
...
void Check_for_File(int N){
    std::string name = "results/"+std::to_string(N)+"ellipse";
    struct stat buffer;   
