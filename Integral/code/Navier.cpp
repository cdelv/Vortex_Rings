...
void Calc_Initial_Velocity(const Vector &r, double t, Vector &u);
...
{   
    ...
    VectorFunctionCoefficient w_bdr(pmesh->Dimension(), Initial_Vorticity);//Initial_Vorticity); //f_exact
    w.ProjectCoefficient(w_bdr);

    //Define Velocity Initial Conditions
    VectorFunctionCoefficient u_init(pmesh->Dimension(), Calc_Initial_Velocity);
