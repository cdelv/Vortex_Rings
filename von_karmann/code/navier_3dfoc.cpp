...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < ctx.serial_refinements; ++i)
   {
   ...
   delete mesh;
    ...
    for (int ii = 0; ii < ctx.parallel_refinements; ii++)
        pmesh->UniformRefinement();

    //Create H1 Element Collections
    H1_FECollection vfec = H1_FECollection(ctx.order, pmesh->Dimension());
    H1_FECollection pfec = H1_FECollection(ctx.order);
   ...
   CurlGridFunctionCoefficient curl_u(u_gf);
   w.ProjectCoefficient(curl_u);

   ParaViewDataCollection pvdc("results", pmesh);
   ...
   pvdc.RegisterField("Vorticity", &w);
   pvdc.Save();
   ...
   for (int step = 0; !last_step; ++step)
   {
      ...
      if (step % ctx.vis_steps == 0)
      {
      	 CurlGridFunctionCoefficient u_curl(u_gf);
